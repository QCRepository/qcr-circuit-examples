...
if __name__ == "__main__":
    ...
    grid_resolution = 1 / (2 ** n_counting)

    print(f"Quantum Phase Estimation — T-gate on |1>\n")
    print(f"  Unitary:             T  (T|1> = e^(2pi*i/8)|1>)")
    print(f"  Counting qubits:     {n_counting}  (resolution: 1/{2 ** n_counting} = {grid_resolution})")
    print(f"  Shots:               {shots}\n")

    # Top 3 measurement outcomes
    top_outcomes = sorted(counts.items(), key=lambda kv: -kv[1])[:3]
    print("Top measurement outcomes:")
    for bitstring, count in top_outcomes:
        measured_int = int(bitstring, 2)
        theta_candidate = measured_int / (2 ** n_counting)
        probability = count / shots
        print(f"  |{bitstring}>  (int={measured_int}, theta={theta_candidate})  "
              f"count={count}  prob={probability:.3f}")

    print(f"\nEstimated phase theta: {theta}")
    print(f"Expected phase theta:  {expected_theta}")
    print(f"Absolute error:        {abs(theta - expected_theta):.6f}  "
